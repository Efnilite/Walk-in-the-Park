 ...
 * @author Efnilite
 */
public class VersionManager_v1_16_R1 implements VersionManager {
    ...
    public void pasteStructure(File file, Location to) {
        try {
            ...
            structure.a(world, pos, info, ThreadLocalRandom.current());
        } catch (IOException e) {
            e.printStackTrace();
        }
    }

    // gets the appropriate rotation for the schematic for the heading
    ...
    public @Nullable DefaultGenerator.StructureData placeAt(File file, Location to, Vector heading) {
        try {
            ...
            structure.a(world, new BlockPosition(to.getX(), to.getY(), to.getZ()), info, ThreadLocalRandom.current());
